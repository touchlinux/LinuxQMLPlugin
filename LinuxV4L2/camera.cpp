...
void Camera::SetDevice(const QString& dev)
{
    if (mDevName != NULL) {
        StopCapture();
        free(mDevName);
...
void Camera::emitError(const char *s)
{
...
QImage Camera::convertFromYUV422(const QByteArray &array, int w, int h)
{
...
void Camera::process_image(const void *p, int size)
{
    if (mOutBuf) {
        QByteArray array;
...
int Camera::stop_capture_device(void)
{
...
int Camera::start_capture_device(void)
{
    unsigned int i;
    enum v4l2_buf_type type;
